<%-- 
    Document   : catViaEmbarque_list
    Created on : 14/08/2015, 05:40:27 PM
    Author     : leonardo
--%>
...
if (user == null || !user.permissionToTopicByURL(request.getRequestURI().replace(request.getContextPath(), ""))) {
    ...
    int idViaEmbarque = -1;
    try{ idViaEmbarque = Integer.parseInt(request.getParameter("idViaEmbarque")); }catch(NumberFormatException e){}
    ...
    */
    ...
    try{
     ...
     ViaEmbarque[] viaEmbarquesDto = new ViaEmbarque[0];
     try{
         limiteRegistros = viaEmbarqueBO.findViaEmbarques(idViaEmbarque, idEmpresa , 0, 0, filtroBusqueda).length;
        ...
        viaEmbarquesDto = viaEmbarqueBO.findViaEmbarques(idViaEmbarque, idEmpresa,
    ...
    * Datos de catálogo
    */
    String urlTo = "../catViaEmbarque/catViaEmbarque_form.jsp";
    String paramName = "idViaEmbarque";
                    ...
                    <div class="onecolumn">
						...
						<tbody>
							<tr>
                                                            <td>
                                                                <div id="search">
                                                                <form action="catViaEmbarque_list.jsp" id="search_form" name="search_form" method="get">
                        ...
                        <div class="content">
                            <form id="form_data" name="form_data" action="" method="post">
                                <table class="data" width="100%" cellpadding="0" cellspacing="0">
                                    ...
                                    <tbody>
                                        <% 
                                            for (ViaEmbarque item:viaEmbarquesDto){
                                                try{
                                        %>
                                        <tr <%=(item.getIdEstatus()!=1)?"class='inactive'":""%>>
                                            <!--<td><input type="checkbox"/></td>-->
                                            <td><%=item.getIdViaEmbarque() %></td>
                                            <td><%=item.getNombre() %></td>
                                            <td><%=item.getDescripcion()%></td>                                            
                                            <td>
                                                <%if(RolAutorizacionBO.permisoAccionesElemento(user.getUser().getIdRoles())){%>
                                                <a href="<%=urlTo%>?<%=paramName%>=<%=item.getIdViaEmbarque()%>&acc=1&pagina=<%=paginaActual%>"><img src="../../images/icon_edit.png" alt="editar" class="help" title="Editar"/></a>
                                                &nbsp;&nbsp;
