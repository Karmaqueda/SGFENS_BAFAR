...
if (user == null || !user.permissionToTopicByURL(request.getRequestURI().replace(request.getContextPath(), ""))) {
    ...
    */
    ...
    try{
     ...
     FoliosMovilEmpleadoBO foliosMovilEmpleadoBO = new FoliosMovilEmpleadoBO(user.getConn());
     FoliosMovilEmpleado[] foliossDto = new FoliosMovilEmpleado[0];
     try{
         limiteRegistros = foliosMovilEmpleadoBO.findCantidadFoliosMovilEmpleado(idFolios, idEmpresa , 0, 0, filtroBusqueda);
        ...
        foliossDto = foliosMovilEmpleadoBO.findFoliosMovilEmpleado(idFolios, idEmpresa,
    ...
    * Datos de catálogo
    */
    String urlTo = "../catFoliosMovil/catFoliosMovil_form.jsp";
    String paramName = "idFolios";
    String parametrosPaginacion="idFolios="+idFolios+"&q="+buscar;// "idEmpresa="+idEmpresa;
                    ...
                    <div class="onecolumn">
                        <div class="header">
                            <span>
                                ...
                                Folios de Control Interno M&oacute;vil
                            </span>
                                ...
                                <table width="500px" cellpadding="0" cellspacing="0">
                                    <tbody>
                                        <tr>
                                            <td>
                                                <div id="search">
                                                <form action="catFoliosMovil_list.jsp" id="search_form" name="search_form" method="get">
                        ...
                        <div class="content">
                            <form id="form_data" name="form_data" action="" method="post">
                                <table class="data" width="100%" cellpadding="0" cellspacing="0">
                                    ...
                                    <tbody>
                                        <% 
                                            for (FoliosMovilEmpleado item : foliossDto){
                                                try{
                                        %>
                                        <tr <%=(item.getIdEstatus()!=1)?"class='inactive'":""%>>
                                            <!--<td><input type="checkbox"/></td>-->
                                            <td><%=item.getIdFolioMovilEmpleado()%></td>
                                            <td><%=item.getSerie() %></td>
                                            <td><%=item.getFolioDesde() %></td>
                                            <td><%=item.getUltimoFolio() %></td>
                                            <!--<td><%=item.getFolioHasta() %></td>--> 
                                            <td>
                                                <%if(RolAutorizacionBO.permisoAccionesElemento(user.getUser().getIdRoles())){%> 
                                                <a href="<%=urlTo%>?<%=paramName%>=<%=item.getIdFolioMovilEmpleado()%>&acc=1&pagina=<%=paginaActual%>">
