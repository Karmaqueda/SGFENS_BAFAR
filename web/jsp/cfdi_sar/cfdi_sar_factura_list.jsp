<%-- 
    Document   : cfdi_sar_factura_list.jsp
    Created on : 20-dic-2015, 12:13:49
...
if (user == null || !user.permissionToTopicByURL(request.getRequestURI().replace(request.getContextPath(), ""))) {
    ...
    String buscar_idestatus_sar = request.getParameter("q_idestatus_sar")!=null?request.getParameter("q_idestatus_sar"):"";
    ...
    //filtro Status SAR
    if (!buscar_idestatus_sar.trim().equals("")){
        filtroBusqueda = " AND sar_comprobante.EXT_SAR_ID_ESTATUS = '" + buscar_idestatus_sar +"' " ;
    }
    
    //Ademas solo Facturas de Tipo FACTURA
    filtroBusqueda += " AND (ID_TIPO_COMPROBANTE='2' OR ID_TIPO_COMPROBANTE='41') ";
    ...
    */
    ...
    try{
     ...
     SarComprobanteBO sarComprobanteBO = new SarComprobanteBO(user.getConn());
     ComprobanteFiscal[] comprobanteFiscalDto = new ComprobanteFiscal[0];
     try{
         limiteRegistros = sarComprobanteBO.findComprobanteFiscal(idComprobanteFiscal, idEmpresa , 0, 0, filtroBusqueda).length;
        ...
        comprobanteFiscalDto = sarComprobanteBO.findComprobanteFiscal(idComprobanteFiscal, idEmpresa,
     ...
     //String listaIDSarComprobante = "";
     //Actualizamos información de registros de SAR
     for (ComprobanteFiscal compFiscal : comprobanteFiscalDto){
         sarComprobanteBO = new SarComprobanteBO(compFiscal.getIdComprobanteFiscal(), user.getConn());
         try { 
             sarComprobanteBO.actualizarComprobanteFromSAR(); 
         }catch(Exception ex){
             ex.printStackTrace();
         }    
     }
    ...
    * Datos de catálogo
    */
    String urlTo = "../cfdi_factura/cfdi_factura_form.jsp";
    String paramName = "idComprobanteFiscal";
    //String parametrosPaginacion="";// "idEmpresa="+idEmpresa;
    String parametrosPaginacion="q="+buscar+"&q_idcliente="+buscar_idcliente+"&q_idestatuscomprobanteFiscal="+buscar_idestatuscomprobanteFiscal+"&q_idsucursal="+buscar_idsucursal+"&q_fh_min="+fechaMin+"&q_fh_max="+fechaMax+"&q_idestatus_sar"+buscar_idestatus_sar;// "idEmpresa="+idEmpresa;
    ...
    <head>
        ...
        <script type="text/javascript">
            ...
            function confirmarCancelarComprobanteFiscal(idComprobanteFiscal, UUID){
                apprise('¿Esta seguro que desea cancelar el Comprobante Fiscal con UUID <br/><b>'+UUID+'</b>?', {'verify':true, 'animate':true, 'textYes':'Si', 'textNo':'No'}, 
                    function(r){
                        if(r){
            ...
            function cancelarComprobanteFiscal(idComprobanteFiscal){
                if(idComprobanteFiscal>=0){
                    $.ajax({
                        type: "POST",
                        url: "cfdi_sar_factura_list_ajax.jsp",
                        data: { mode: '2', id_comprobanteFiscal : idComprobanteFiscal },
                        ...
                        success: function(datos){
                            if(datos.indexOf("--EXITO-->", 0)>0){
                               ...
                               apprise('<center><img src=../../images/info.png> <br/>'+ datos +'</center>',{'animate':true},
                                    function(r){
                                        location.href = "cfdi_sar_factura_list.jsp?idComprobanteFiscal="+idComprobanteFiscal;
                                });
                           }else{
                               $("#ajax_loading").fadeOut("slow");
                               //$("#ajax_message").html(datos);
                ...
                <div class="inner">
                    ...
                    <div class="onecolumn">
                        ...
                        <div class="content" style="display: none;">
                            <form action="cfdi_sar_factura_list.jsp" id="search_form_advance" name="search_form_advance" method="post">
                                <p>
                                ...
                                <p>
                                <label>Estatus Pretoriano:</label><br/>
                                ...
                                <p>
                                <label>Estatus SAR:</label><br/>
                                <select id="q_idestatus_sar" name="q_idestatus_sar">
                                    <option></option>
                                    <option value="1">Inválida/Cancelada</option>
                                    <option value="2">Recepción</option>
                                    <option value="3">Recepción con Reenvio</option>
                                    <option value="4">Rechazo (Aprobador)</option>
                                    <option value="5">Aprobación</option>
                                    <option value="6">Rechazo (Cuentas por Pagar)</option>
                                    <option value="7">Aprobada (Cuentas por Pagar)</option>
                    ...
                    <div class="onecolumn">
                                ...
                                <table width="600px" cellpadding="0" cellspacing="0">
                                    <tbody>
                                        <tr>
                                            <td>
                                                <div id="search">
                                                <form action="cfdi_sar_factura_list.jsp" id="search_form" name="search_form" method="get">
                                            ...
                                            <td class="clear">&nbsp;&nbsp;&nbsp;</td>
                                            <td>
                                                <input type="button" id="nuevo" name="nuevo" class="right_switch" value="Nuevo CFDI SAR" 
                                                        style="float: right; width: 100px;" onclick="location.href='<%=urlTo%>?acc=1&endAccName=cfdi_sar'"/>
                                            </td>
                                        </tr>
                                    </tbody>
                                </table>
                        ...
                        <div class="content">
                                ...
                                <table class="data" width="100%" cellpadding="0" cellspacing="0">
                                    ...
                                    <tbody>
                                        <% 
                                            ...
                                            for (ComprobanteFiscal item:comprobanteFiscalDto){
                                                try{
                                                    ...
                                                    sarComprobanteBO = new SarComprobanteBO(item.getIdComprobanteFiscal(), user.getConn());
                                                    SarComprobante sarComprobanteDto = sarComprobanteBO.getSarComprobante();
                                                    //if (sarComprobanteDto.getExtSarIdFactura()>0)
                                                    //    listaIDSarComprobante += sarComprobanteDto.getExtSarIdFactura() + ",";
                                                    
                                                    //Consulta a tabla de Adjuntos del comprobante para saber cuantos tiene
                                                    int noAdjuntos = sarComprobanteAdjuntoBO.getCantidadAdjuntosByComprobanteFiscal(item.getIdComprobanteFiscal());
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                <% if (sarComprobanteDto!=null && sarComprobanteDto.getExtSarIdFactura()>0){  
                                                    out.print(sarComprobanteDto.getExtSarDescEstatus());
                                                }else{
                                                %>
                                                <b><i>No Enviada</i></b>
                                                &nbsp;&nbsp;
                                                <!-- Enviar a SAR -->
                                                <a href="../cfdi_sar/cfdi_sar_factura_campos_extra_form.jsp?idComprobanteFiscal=<%=item.getIdComprobanteFiscal() %>"><img src="../../images/sar_enviar.png" alt="Enviar SAR" class="help" title="Enviar SAR" /></a>
                                                <%}%>
                                            </td>
                                            ...
                                            <td>
                                                ...
                                                <!-- Ver Adjuntos SAR-->
                                                <% if (noAdjuntos<=0){ %>
                                                <img src="../../images/sar_adjunto_vacio.png" width="16" height="16" alt="<%=noAdjuntos%> Adjuntos" class="help" title="<%=noAdjuntos%> Adjuntos" />
                                                <% }else{ %>
                                                <a class="modalbox_iframe" href="../cfdi_sar/cfdi_sar_adjuntosfactura_ajax.jsp?idComprobanteFiscal=<%=item.getIdComprobanteFiscal() %>"
                                                   title="Archivos Adjuntos de la Factura ID: <%=item.getIdComprobanteFiscal()%>" >
                                                    <img src="../../images/sar_adjunto_varios.png" width="16" height="16" alt="<%=noAdjuntos%> Adjuntos" class="help" title="<%=noAdjuntos%> Adjuntos" />
                                                </a>
                                                <% } %>
                                                &nbsp;&nbsp;
                                                <% if (sarComprobanteDto!=null && sarComprobanteDto.getExtSarIdFactura()>0){  %>
                                                <!-- Ver Workflow -->
                                                <a class="modalbox_iframe" href="../cfdi_sar/cfdi_sar_flujofactura_ajax.jsp?idComprobanteFiscal=<%=item.getIdComprobanteFiscal() %>"
                                                   title="Flujo de la Factura en SAR: <%=item.getIdComprobanteFiscal()%>" >
                                                    <img src="../../images/sar_workflow_icon.png" alt="Flujo SAR" class="help" title="Flujo SAR" />
                                                </a>
                                                &nbsp;&nbsp;
                                                <%      if (sarComprobanteDto.getExtSarIdEstatus()==SarComprobanteBO.FACTURA_EN_FLUJO_REENVIADA
                                                                || sarComprobanteDto.getExtSarIdEstatus()==SarComprobanteBO.FACTURA_EN_FLUJO){  %>
                                                <!-- Editar OC o Adjuntos -->
                                                <a href="../cfdi_sar/cfdi_sar_factura_campos_extra_form.jsp?acc=2&idComprobanteFiscal=<%=item.getIdComprobanteFiscal() %>"><img src="../../images/icon_edit.png" alt="Editar Datos y Adjuntos" class="help" title="Editar Datos y Adjuntos" /></a>
                                                <%      } %>
                                                <% } %>
                                                <!--FIN Acciones SAR ----->
                            ...
                            <jsp:include page="../include/reportExportOptions.jsp" flush="true">
                                <jsp:param name="idReport" value="<%= ReportBO.FACTURA_SAR_REPORT %>" />
