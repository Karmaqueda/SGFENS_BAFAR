<%-- 
    Document   : cbb_factura_ajax
    Created on : 28/05/2013, 11:15:20 AM
    Author     : Leonardo
--%>
    ...
    */
    ...
    if (mode==-1){
        ...
        if (!genericValidator.isValidString(comentarios, 0, 500))
            msgError = "<ul>El campo comentarios del comprobante CBB es incorrecto. Máximo 500 caracteres. ";
        
        if (idCliente<=0)
            msgError+="<ul> Debe seleccionar un cliente para generar el comprobante CBB. ";
        ...
        if (!genericValidator.isValidString(tipoPagoDescripcion, 1, 100))
            msgError+="<ul> El campo 'Método de Pago' es incorrecto. Mínimo 1 caracter, máximo 100.";
        ...
        //validamos que tengan un folio seleccionado:
         if (idFolios<=0)
            msgError+="<ul> Debe seleccionar una serie para generar el comprobante CBB. ";
        ...
        if (msgError.equals("")){
            if (cfdiSesion==null){
                msgError += "<ul>No se ha inicializado el Comprobante CBB, esta vacío. ";
            }else{
                ...
                if (msgError.equals("") && validador!=0){
                    ...
                    if (msgError.equals("")){
                        ...
                        try{
                            comprobanteFiscalDto = comprobanteFiscalBO.creaCBBActualizaComprobanteFiscal(cfdiSesion, user);
                            ...
                            msgError+= "<ul>Ocurrio un error al almacenar el registro de comprobante CBB: <br/>" + ex.toString();                            
                        }
                    }
                ...
                if (msgError.equals("")){
                    out.print("Comprobante CBB almacenado satisfactoriamente. <br/>");
                ...
                if (msgError.equals("")){
                    try{
                        if (enviarCorreo){
                            ...
                            out.print("<ul>"+ comprobanteFiscalBO.enviaNotificacionNuevaComprobanteFiscal(listCorreosValidos,null, archivoPDF,idEmpresa));
                        }
                    }catch(Exception ex){
        ...
        if (id_comprobanteFiscal>=0){
            ...
            try{
                if (cfdiSesionBO.cancelaComprobanteFiscalCBB()){
