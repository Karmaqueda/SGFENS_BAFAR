...
if (user == null || !user.permissionToTopicByURL(request.getRequestURI().replace(request.getContextPath(), ""))) {
    ...
    int tipoComp = -1;
    String tipoCompr = "";
    try{ tipoComp = Integer.parseInt(request.getParameter("tipoComp")); 
        if(tipoComp==5)
            tipoCompr = "de Crédito";
        else if(tipoComp==6)
            tipoCompr = "de Débito";
    }catch(NumberFormatException e){}
        
    //Ademas solo Comprobantes de Tipo Notas y filtrado por el tipo (crédito, débito)
    if(tipoComp>0)
        filtroBusqueda += " AND ID_TIPO_COMPROBANTE='"+tipoComp+"' ";
    ...
    * Datos de catálogo
    */
    String urlTo = "../cfdi_factura/cfdi_nota_form.jsp";
    String paramName = "idComprobanteFiscal";
    //String parametrosPaginacion="";// "idEmpresa="+idEmpresa;
    String parametrosPaginacion="q="+buscar+"&q_idcliente="+buscar_idcliente+"&q_idestatuscomprobanteFiscal="+buscar_idestatuscomprobanteFiscal+"&q_idsucursal="+buscar_idsucursal+"&q_fh_min="+fechaMin+"&q_fh_max="+fechaMax;// "idEmpresa="+idEmpresa;
                                ...
                                <table width="500px" cellpadding="0" cellspacing="0">
                                    <tbody>
                                        <tr>
                                            <td>
                                                <div id="search">
                                                    <form action="cfdi_nota_list.jsp?tipoComp=<%=tipoComp%>" id="search_form" name="search_form" method="get">
                                            ...
                                            <td class="clear">&nbsp;&nbsp;&nbsp;</td>
                                            <td>
                                                <input type="button" id="nuevo" name="nuevo" class="right_switch" value="Nueva Nota" 
                                                        style="float: right; width: 100px;" onclick="location.href='<%=urlTo%>?acc=1&tipoComp=<%=tipoComp%>'"/>
                                            </td>
                                        </tr>
                                    </tbody>
                                </table>
