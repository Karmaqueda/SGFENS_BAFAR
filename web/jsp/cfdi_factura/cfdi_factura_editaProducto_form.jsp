<%-- 
    Document   : cfdi_factura_editaProducto_form.jsp
...
<%
    ...
    almacenNombre = request.getParameter("nomAlm") != null ? request.getParameter("nomAlm") : "";almacenNombre = request.getParameter("nomAlm") != null ? request.getParameter("nomAlm") : "";
    
    ProductoSesion productoSesion = null;
    int idTipoComprobante = 2;
    int mode = 8 ;
    
    if (cfdiSesion!=null){
        idTipoComprobante = cfdiSesion.getId_tipo_comprobante();       
        
        if (cfdiSesion.getListaProducto()!=null){
            if (cfdiSesion.getListaProducto().size()>0){
                try{
                    productoSesion = cfdiSesion.getListaProducto().get(index_lista_producto);                    
                }catch(Exception e){
        ...
        msgError += "<ul>No existe ningun Comprobante Fiscal en sesion";
    }
    ...
    /*if(idTipoComprobante == 5 || idTipoComprobante == 6){
        mode = 23;
    }    */ 
...
<html>
    <head>
        ...
        <title>Editar Campo de Comprobante Fiscal</title>
        ...
        <script language="javascript">
            function modificarProducto(){
                ...
                producto_almacen = $('#producto_almacen').val();
                
                
                if (validarAddProducto(indexproducto,productoId,productoCantidad,productoMonto, unidadId,producto_almacen,'<%=almacenNombre%>')){
                    if(productoId>0){
                        $.ajax({
                            type: "POST",
                            url: "cfdi_factura_ajax.jsp",
                            data: { mode: '<%=mode%>', index_lista_producto: indexproducto, producto_id : productoId, 
                                producto_cantidad : productoCantidad, producto_monto : productoMonto,
                                producto_precio : productoPrecio, producto_unidad : unidadText , id_tipo_comprobante : '<%=idTipoComprobante%>',
                                producto_almacen : producto_almacen,almacenNombre : '<%=almacenNombre%>'},
            ...
            function validarAddProducto(indexProducto,idProducto,cantidad,monto,idUnidad,producto_almacen,almacenNombre){
                if(indexProducto<0 || idProducto<=0 || cantidad<0 || monto<0 || idUnidad<=0 || (producto_almacen <=0 && almacenNombre !='Sin Almacen')
                    || cantidad=="" || monto==""){
                    ...
                    <form action="" method="post" id="frm_action">
                        ...
                        <fieldset>
                            ...
                            <p>
                                ...
                                <input type="hidden" maxlength="8" name="producto_almacen" id="producto_almacen"
