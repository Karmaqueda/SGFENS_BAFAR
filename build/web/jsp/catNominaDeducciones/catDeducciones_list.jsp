<%-- 
    Document   : catDeducciones_list
    Created on : 7/02/2014, 12:00:23 PM
    Author     : leonardo
--%>
...
if (user == null || !user.permissionToTopicByURL(request.getRequestURI().replace(request.getContextPath(), ""))) {
    ...
    try{ idNominaDeduccion = Integer.parseInt(request.getParameter("idNominaDeduccion")); }catch(NumberFormatException e){}
    ...
    */
    ...
    try{
     ...
     NominaDeduccion[] nominaDeduccionsDto = new NominaDeduccion[0];
     try{
         limiteRegistros = nominaDeduccionBO.findNominaDeduccions(idNominaDeduccion, idEmpresa , 0, 0, filtroBusqueda).length;
        ...
        nominaDeduccionsDto = nominaDeduccionBO.findNominaDeduccions(idNominaDeduccion, idEmpresa,
    ...
    * Datos de catálogo
    */
    String urlTo = "../catNominaDeducciones/catDeducciones_form.jsp";
    String paramName = "idNominaDeduccion";
                    ...
                    <div class="onecolumn">
						...
						<tbody>
							<tr>
                                                            <td>
                                                                <div id="search">
                                                                <form action="catDeducciones_list.jsp" id="search_form" name="search_form" method="get">
                        ...
                        <div class="content">
                            <form id="form_data" name="form_data" action="" method="post">
                                <table class="data" width="100%" cellpadding="0" cellspacing="0">
                                    ...
                                    <tbody>
                                        <% 
                                            for (NominaDeduccion item:nominaDeduccionsDto){
                                                try{
                                        %>
                                        <tr <%=(item.getIdEstatus()!=1)?"class='inactive'":""%>>
                                            <!--<td><input type="checkbox"/></td>-->
                                            <td><%=item.getIdNominaDeduccion() %></td>
                                            <td><%=item.getClave() %></td>
                                            <td><%=item.getConcepto()%></td>                                            
                                            <td><%=item.getImporteGravado() %></td>
                                            <td><%=item.getImporteExcepto() %></td>
                                            <td>
                                                <a href="<%=urlTo%>?<%=paramName%>=<%=item.getIdNominaDeduccion()%>&acc=1&pagina=<%=paginaActual%>"><img src="../../images/icon_edit.png" alt="editar" class="help" title="Editar"/></a>
                                                &nbsp;&nbsp;
