...
<%
    ...
    */
    ...
    if (mode==-1){
        ...
        //variable para el control de saber que es un pedido generado para ser usado como base:
        int pedidoBaseRegla = 0;
        try{ pedidoBaseRegla = Integer.parseInt(request.getParameter("pedidoBaseRegla")); }catch(Exception e){}
        
        //nuevo tipo de idEstatus para saber que es un pedido de tipo base para una regla de cobranza
        if(pedidoBaseRegla > 0){
            idEstatus = 10;
        }
        
        
        if (pedido_fecha_generacion!=null){
            //Le asignamos hora, fecha y minuto actual, a la fecha elegida de generacion de Pedido
            Calendar cal = Calendar.getInstance();
            Calendar cal2 = Calendar.getInstance();
            cal.setTime(pedido_fecha_generacion);
            
            cal.set(Calendar.HOUR_OF_DAY,cal2.get(Calendar.HOUR_OF_DAY));
            cal.set(Calendar.MINUTE,cal2.get(Calendar.MINUTE));
            cal.set(Calendar.SECOND,cal2.get(Calendar.SECOND));
            
            pedido_fecha_generacion = cal.getTime();
        }
        ...
        if (msgError.equals("")){
            if (pedidoSesion==null){
               ...
               /*if (msgError.trim().equals("") && pedidoSesion.getIdPedido()<=0){
                ...
                System.out.println("------------- ID DE PEDIDO INSERTADO: " + pedidoDto.getIdPedido());
                if (msgError.equals("")){
                    out.print("Pedido almacenado satisfactoriamente. <br/><br/><b> Folio de Pedido: " + pedidoDto.getFolioPedido() + "</b>");
                    
                    if(pedidoBaseRegla > 0){
                        //ALMACENAMOS EL ID DEL PEDIDO GENERADO, ESTO PARA OBTENER LA RELACION DE QUE ID DE PEDIDO BASE SE GENERO; ESTO SOLO PARA EL PEDIDO BASE DE REGLA DE COBRANZA
                        request.getSession().setAttribute("idPedidoBaseGenerado", pedidoDto.getIdPedido());
                    }
...
%>
                        <tr>
                            ...
                            <td>
                                <a id="almacen_ref" class="modalbox_iframe" href="../../jsp/includeProductoSelect/almacenEdit_select_form.jsp?mode=100&idProducto=<%=concepto.getIdConcepto()%>&identificadorIdAlmacen=almacen_id_<%=id%>&identificadorNombreAlmacen=almacen_nombre_<%=id%>&index_lista_producto=<%=id%>">
                                    <img src="../../images/icon_almacen1.png" alt="Modificar Almacen" title="Modificar Almacen" class="help" height="20" width="20"/>
                                </a>
                            </td>
                            <%//}%>
