...
<%
    try{
        ...
        int isXlsSmsDestinatarios = -1;
        try{
            isXlsSmsDestinatarios = Integer.parseInt(request.getParameter("isXlsSmsDestinatarios"));
        }catch(Exception ex){}
        
        int isImagenPDFContrato = -1;
        try{
            isImagenPDFContrato = Integer.parseInt(request.getParameter("isImagenPDFContrato"));
        }catch(Exception ex){}
        
        int isImagenCrFormularioAdjunto = -1;
        try{
            isImagenCrFormularioAdjunto = Integer.parseInt(request.getParameter("isImagenCrFormularioAdjunto"));
        }catch(Exception ex){}
        int isDocGenericoModuloCR = -1;
        try{
            isDocGenericoModuloCR = Integer.parseInt(request.getParameter("isDocGenericoModuloCR"));
        }catch(Exception ex){}
                ...
                if (!fileItem.isFormField()) {
                    ...
                    if(archivoValido){
                        ...
                        if (isCertificadoAdjunto>0 || isKeyAdjunto>0 || isImagenPersonalAdjunto>0 || isCbbAdjunto > 0 ){
                            ...
                            ourRepositoryDirectory += empresaDto.getRfc() + "/";
                        }else if(isXlsSmsDestinatarios==1){ //renombramos archivo cuando es carga de destinatarios SMS por excel, para sobreescribirlo y no almacenarlos                         
                            
                            Configuration configuration = new Configuration();                                    
                            File folder = new File(configuration.getApp_content_path()+empresaDto.getRfc());
                            folder.mkdirs();                                                     
                            
                            uploadedFilename = "cargaSMSDestinatarios.xls";
                            ourRepositoryDirectory += empresaDto.getRfc() + "/";
                        }else if(isImagenPDFContrato > 0){
                            //CREAMOS EL DIRECTORIO:
                            Configuration configuration = new Configuration();                                    
                            File folder = new File(configuration.getApp_content_path()+empresaDto.getRfc()+"/ContratoComodato");
                            folder.mkdirs();
                            ourRepositoryDirectory += empresaDto.getRfc()+"/ContratoComodato/";                            


                        }else if(isImagenCrFormularioAdjunto > 0){
                            //CREAMOS EL DIRECTORIO:
                            Configuration configuration = new Configuration();                                    
                            File folder = new File(configuration.getApp_content_path()+empresaDto.getRfc()+"/AdjuntosFormulario");
                            folder.mkdirs();
                            ourRepositoryDirectory += empresaDto.getRfc()+"/AdjuntosFormulario/";  
                        }else if(isDocGenericoModuloCR > 0){
                            Configuration configuration = new Configuration();                                    
                            File folder = new File(configuration.getApp_content_path()+empresaDto.getRfc()+"/CrDocumentos");
                            folder.mkdirs();
                            ourRepositoryDirectory += empresaDto.getRfc()+"/CrDocumentos/";  
                        }
                        ...
                        if(isImagenConceptoAdjunto > 0 || isImagenProveedorAdjunto > 0 || isImagenPDFContrato > 0 || isImagenCrFormularioAdjunto > 0){
                        ...
                        if(isImagenConceptoAdjunto>0 || isImagenProveedorAdjunto > 0 || isImagenPDFContrato > 0){
                        ...
                        if (isXlsSmsDestinatarios==1){                         
                            out.print("<script>parent.recuperarNombreArchivoXLS('" + new String(fileItem.getName().getBytes("ISO-8859-1"),"UTF-8") + "');</script>");
                        }
                        
                        if(isImagenCrFormularioAdjunto>0){
                            String IdCampoFormulario = request.getParameter("IdCampoFormulario")!=null?request.getParameter("IdCampoFormulario"):"";
                            System.out.println("+**++**+*+*+*+*+ ID DE IMAGEN CAMPO D: "+ IdCampoFormulario);
                            //Agrega el nombre del KEY al formulario padre, por medio de un metodo predefinido en el parent con javascript
                            out.print("<script>parent.recuperarNombreArchivoImagen('"+fechaConcatenarNombre + new String(fileItem.getName().getBytes("ISO-8859-1"),"UTF-8") + "', '" + IdCampoFormulario + "');</script>");
                        }
                        
                        if(isDocGenericoModuloCR>0){
                            //Agrega el nombre del Archivo al formulario padre, por medio de un metodo predefinido en el parent con javascript
                            out.print("<script>parent.recuperarNombreArchivo('" + new String(fileItem.getName().getBytes("ISO-8859-1"),"UTF-8") + "', " + isDocGenericoModuloCR + ");</script>");
                        }
