<%-- 
    Document   : catBanco_list
    Created on : 12/12/2014, 01:54:56 PM
    Author     : leonardo
--%>
...
if (user == null || !user.permissionToTopicByURL(request.getRequestURI().replace(request.getContextPath(), ""))) {
    ...
    if (!buscar.trim().equals(""))
        filtroBusqueda = " AND (NOMBRE_BANCO LIKE '%" + buscar + "%' OR COMENTARIOS LIKE '%" +buscar+"%')";
    
    int idSgfensBanco = -1;
    try{ idSgfensBanco = Integer.parseInt(request.getParameter("idSgfensBanco")); }catch(NumberFormatException e){}
    ...
    */
    ...
    try{
     ...
     SgfensBanco[] sgfensBancosDto = new SgfensBanco[0];
     try{
         limiteRegistros = sgfensBancoBO.findSgfensBancos(idSgfensBanco, idEmpresa , 0, 0, filtroBusqueda).length;
        ...
        sgfensBancosDto = sgfensBancoBO.findSgfensBancos(idSgfensBanco, idEmpresa,
    ...
    * Datos de catálogo
    */
    String urlTo = "../catBancos/catBanco_form.jsp";
    String paramName = "idSgfensBanco";
                                ...
                                <table width="500px" cellpadding="0" cellspacing="0">
                                    <tbody>
                                            <tr>
                                                <td>
                                                    <div id="search">
                                                    <form action="catBanco_list.jsp" id="search_form" name="search_form" method="get">
                        ...
                        <div class="content">
                            <form id="form_data" name="form_data" action="" method="post">
                                <table class="data" width="100%" cellpadding="0" cellspacing="0">
                                    ...
                                    <tbody>
                                        <% 
                                            for (SgfensBanco item:sgfensBancosDto){
                                                try{
                                        %>
                                        <tr <%=(item.getIdEstatus()!=1)?"class='inactive'":""%>>
                                            <!--<td><input type="checkbox"/></td>-->
                                            <td><%=item.getIdBanco() %></td>
                                            <td><%=item.getNombreBanco()%></td>
                                            <td><%=item.getNumeroCuenta()%></td>
                                            <td><%=item.getComentarios()%></td>
                                            <td>
                                                <%if(RolAutorizacionBO.permisoAccionesElemento(user.getUser().getIdRoles())){%>
                                                <a href="<%=urlTo%>?<%=paramName%>=<%=item.getIdBanco()%>&acc=1&pagina=<%=paginaActual%>"><img src="../../images/icon_edit.png" alt="editar" class="help" title="Editar"/></a>
                                                &nbsp;&nbsp;                                                
