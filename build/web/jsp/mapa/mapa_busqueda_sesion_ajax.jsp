...
<%
    ...
    */
    ...
    }else if(mode.equals("guardarVariableEnSesion")){        
        try{        
            String nombre = request.getParameter("nombre")!=null?new String(request.getParameter("nombre").getBytes("ISO-8859-1"),"UTF-8"):"";
            String valor = request.getParameter("valor")!=null?new String(request.getParameter("valor").getBytes("ISO-8859-1"),"UTF-8"):"";
        
            if (StringManage.getValidString(nombre).length()>0){
                //System.out.println("VAriable de sesion guardada: " + nombre + ", valor: " + valor);
                request.getSession().setAttribute(nombre, valor);
            }
            out.print("<!--EXITO-->Selecci&oacute;n almacenada en sesión.<br/>");
        }catch(Exception e){
            e.printStackTrace();
            String msgError = "Ocurrio un error al guardar selección en Sesion " + e.toString() ;
            out.print("<!--ERROR-->"+msgError);
        }
    }
