...
if (user == null || !user.permissionToTopicByURL(request.getRequestURI().replace(request.getContextPath(), ""))) {
    ...
    int menu = 0;
    try {
        idUsuario = Integer.parseInt(request.getParameter("q_idvendedor"));        
    ...
    if(idUsuario==0){
        try {
            if(menu==0){			
                idEmpleado = Integer.parseInt(request.getParameter("q_idvendedor"));
            }else{
        ...
        idEmpleado = new EmpleadoBO(user.getConn()).findEmpleadoByUsuario(idUsuario).getIdEmpleado();    
    ...
    try {
        empleadoConsultado = empleadoBO.findEmpleadobyId(idEmpleado);
        nombreEmpleado = empleadoConsultado.getNombre() +" "+ empleadoConsultado.getApellidoPaterno() +" "+ empleadoConsultado.getApellidoMaterno();
    } catch (Exception e) {
    }
           ...
           var isPaused = true;
             ...
             <%
                ...
                for(Cliente clienteDto:clientesDtos){
                    if(clienteDto.getLatitud()!=null && clienteDto.getLongitud()!=null){                      
                        String nombreCliente = StringManage.getValidString(clienteDto.getRazonSocial()).replaceAll("(\\r|\\n)", ""); //eliminamos espacios en blanco al inicio y final, y saltos de linea
                ...
                for(SgfensProspecto prospectoDto:prospectosDto){
                    String nombreCliente = StringManage.getValidString(prospectoDto.getContacto()).replaceAll("(\\r|\\n)", ""); //eliminamos espacios en blanco al inicio y final, y saltos de linea
                                    ...
                                    <form action="catEmpleados_RutaDia.jsp?<%=parametrosPaginacion%>" id="search_form_advance" name="search_form_advance" method="post"> 
                                        ...
                                        Empleado
                                                    <select id="q_idvendedor" name="q_idvendedor" class="flexselect">
                                                        <option></option>
                                                        <%= new UsuariosBO().getUsuariosByRolHTMLCombo(idEmpresa, RolesBO.ROL_OPERADOR_VENDEDOR_MOVIL, idUsuario) %> 
                                                        <%= new UsuariosBO().getUsuariosByRolHTMLCombo(idEmpresa, RolesBO.ROL_CONDUCTOR_VENDEDOR, idUsuario) %>
                                                    </select>
                                        </p> 
                                        <br>
                                        <%}%>
