...
if (user == null || !user.permissionToTopicByURL(request.getRequestURI().replace(request.getContextPath(), ""))) {
    ...
    int idEmpleado = -1;
    try{ 
        idEmpleado = Integer.parseInt(request.getParameter("idEmpleado")); }catch(NumberFormatException e){}    
        int idEmpresa = user.getUser().getIdEmpresa();
        
        filtroBusqueda += "  AND ((ID_EMPLEADO_EMISOR IN (SELECT ID_EMPLEADO FROM EMPLEADO WHERE ID_EMPRESA =" + idEmpresa + ")) OR "
			+ " (ID_EMPLEADO_RECEPTOR IN (SELECT ID_EMPLEADO FROM EMPLEADO WHERE ID_EMPRESA =" + idEmpresa + ")) )";
                                            ...
                                            for (MovilMensaje item:movilMensajesDto){
                                                try{
                                                    if(item.getEmisorTipo()==1){
                                                        ebo = new EmpleadoBO(item.getIdEmpleadoEmisor(),user.getConn());
                                                        emplea = ebo.getEmpleado();
                                                        emisorEmpleado = emplea.getNumEmpleado();                                                                                                               
                                                    }else
                                                        emisorEmpleado = "";
                                                    if(item.getReceptorTipo()==1){
                                                        ebo = new EmpleadoBO(item.getIdEmpleadoReceptor(),user.getConn());
                                                        emplea = ebo.getEmpleado();
