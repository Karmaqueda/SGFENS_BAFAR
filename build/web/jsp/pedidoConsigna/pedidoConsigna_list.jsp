<%-- 
    Document   : pedidoConsigna_list
    Created on : 24/11/2015, 04:50:24 PM
    Author     : leonardo
--%>
...
if (user == null || !user.permissionToTopicByURL(request.getRequestURI().replace(request.getContextPath(), ""))) {
    ...
    String filtroBusqueda = " AND (ID_ESTATUS_PEDIDO = 5 OR ID_ESTATUS_PEDIDO = 6 OR CONSIGNA = 2) ";
                ...
                <div class="inner">
                    ...
                    <div class="onecolumn">
                        ...
                        <div class="content" style="display: none;">
                            <form action="pedidoConsigna_list.jsp" id="search_form_advance" name="search_form_advance" method="post">
                                <p>
                    ...
                    <div class="onecolumn">
                        <div class="header">
                            <span>
                                <img src="../../images/icon_ventas4.png" alt="icon"/>
                                Venta en Consigna
                            </span>
                                ...
                                <table width="650px" cellpadding="0" cellspacing="0">
                                        <tbody>
                                                <tr>
                                                    ...
                                                    <td>
                                                        <div id="search">                                                                
                                                        <form action="pedidoConsigna_list.jsp" id="search_form" name="search_form" method="get">
                                ...
                                <table class="data" width="100%" cellpadding="0" cellspacing="0">
                                    ...
                                    <tbody>
                                        <% 
                                            ...
                                            for (SgfensPedido item:pedidoDto){
                                                ...
                                                try{
                                                    ...
                                                    try{ cliente = new ClienteBO(item.getIdCliente(),user.getConn()).getCliente(); 
                                                    ...
                                                    //(del ejemplo de ayer total 100, 10% comisión a pagar 90)
                                                    //en subtotal poner los $100, y en total los 90
                                                    //porque la comisión puede cambiar
                                                    
                                                    if(item.getBonificacionDevolucion()>0){
                                                       //utilidadNeta = (item.getTotal() - costoVentaProducto - comisionConsignatario);
                                                       utilidadNeta = (item.getTotal());
                                                       //ventaTotal = item.getTotal();
                                                       ventaTotal = item.getSubtotal();
                                                       //adeudo = item.getTotal() - item.getSaldoPagado();
                                                    }else{
                                                       //utilidadNeta = ((item.getTotal() + Math.abs(item.getBonificacionDevolucion())) - costoVentaProducto - comisionConsignatario);
                                                       utilidadNeta = (item.getTotal() + Math.abs(item.getBonificacionDevolucion()));
                                                       //ventaTotal = item.getTotal() + Math.abs(item.getBonificacionDevolucion());
                                                       ventaTotal = item.getSubtotal() + Math.abs(item.getBonificacionDevolucion());
                                                       //adeudo = (item.getTotal() + Math.abs(item.getBonificacionDevolucion())) - item.getSaldoPagado();
                                                    }
                                        ...
                                        <tr>
                                            ...
                                            <td><%= item.getIdEstatusPedido()==5?"Si":(item.getIdEstatusPedido()==6?"No":"Si") %></td>
                                            <td><%= (item.getIdEstatusPedido()!=5&&item.getIdEstatusPedido()!=6)?"Cerrado":"Abierto" %></td>
                                            ...
                                            <!--<td><%=formatMoneda.format(comisionConsignatario)%></td>-->
                                            <td><%=formatMoneda.format(utilidadNeta)%></td>
                                            <td>
                                                <%if(RolAutorizacionBO.permisoAccionesElemento(user.getUser().getIdRoles())){%>
                                                
                                                <%if ((item.getIdEstatusPedido()== SGEstatusPedidoBO.ESTATUS_CONSIGNA || item.getIdEstatusPedido() == SGEstatusPedidoBO.ESTATUS_CONSIGNA_PARCIAL)){%>
                                                         <a href="#" onclick="editarPedido(<%=item.getIdPedido()%>);"><img src="../../images/icon_edit.png" alt="editar" class="help" title="Cerrar venta" /></a>                                                          
                                                 <% }  %>                                                
                                                    
                                                    <% if ((item.getIdEstatusPedido()== SGEstatusPedidoBO.ESTATUS_CONSIGNA || item.getIdEstatusPedido() == SGEstatusPedidoBO.ESTATUS_CONSIGNA_PARCIAL)){   //si es de movil y no se ha entregado%>  
                                                        &nbsp;&nbsp;
                                                    ...
                                                    &nbsp;&nbsp;-->
                                                    <% if (item.getIdEstatusPedido()==SGEstatusPedidoBO.ESTATUS_PENDIENTE  || item.getIdEstatusPedido()==SGEstatusPedidoBO.ESTATUS_ENTREGADO_PARCIAL || item.getIdEstatusPedido()==SGEstatusPedidoBO.ESTATUS_CONSIGNA || item.getIdEstatusPedido()==SGEstatusPedidoBO.ESTATUS_CONSIGNA_PARCIAL) {%>
                                                        ...
                                                        <%if (item.getConsigna()==0 || item.getConsigna()==2){ %>
                                                        &nbsp;&nbsp;
                                                        <a href="../pedido/pedido_AsignarPedido_form.jsp?idPedido=<%=item.getIdPedido() %>" id="asignarPedido" title="Asignar Pedido" class="modalbox_iframe">
