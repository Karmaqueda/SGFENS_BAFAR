...
<%@page import="com.tsp.sct.dao.dto.ClientePk"%>
...
<%
    ...
    */
    ...
    double creditoMontoMax = 0;
    int creditoDiasMax = 0;
    ...
    */
    ...
    try{
        latitud = request.getParameter("latitud")!=""?new String(request.getParameter("latitud").getBytes("ISO-8859-1"),"UTF-8"):"0";
        longitud = request.getParameter("longitud")!=""?new String(request.getParameter("longitud").getBytes("ISO-8859-1"),"UTF-8"):"0";
    }catch(Exception ex){}   
    try{
    ...
    double porcentajePermisoVentaCredito = 0;
    try{
        porcentajePermisoVentaCredito = Double.parseDouble(request.getParameter("comisionCli"));
    }catch(Exception ex){}
    
    try{
        creditoMontoMax = Double.parseDouble(request.getParameter("creditoMontoMax"));
    }catch(Exception ex){}
    try{
        creditoDiasMax = Integer.parseInt(request.getParameter("creditoDiasMax"));
    }catch(Exception ex){}
   ...
   ////--**
    ...
    if(mode.equals("express")){ //si el cliente será creado como cliene express
                ...
                */
                ...
                try {
                    ...
                    if(idCliente>0){//edita express
                        ...
                        clienteDto.setComisionConsigna(porcentajePermisoVentaCredito);
                        clienteDto.setNombreComercial(nombreComercial); 
                        clienteDto.setReferencia(referencia);
                        clienteDto.setCreditoMontoMax(creditoMontoMax);
                        clienteDto.setCreditoDiasMax(creditoDiasMax);
                        ...
                        clienteDto.setIdClienteCategoria(idClienteCategoria);
                        
                        clienteDto.setClave(claveCliente);
                        try{
                            clienteDto.setFechaRegistro(ZonaHorariaBO.DateZonaHorariaByIdEmpresa(user.getConn(), new Date(), (int)idEmpresa).getTime());
                        }catch(Exception e){
                        ...
                        if (idVendedor>0){
                            SGClienteVendedorBO clienteVendedorBO = new SGClienteVendedorBO(idCliente,user.getConn());
                           ...
                           if (clienteVendedorDto!=null){
                                ...
                                clienteVendedorDto.setIdUsuarioVendedor(idVendedor);
    ...
    }else if(mode.equals("desactivar")){
        ...
        try{
            if(idCliente>0){
               ...
               out.print("<!--ERROR-->No se pudo eliminar el cliente. Informe del error al administrador del sistema.");
            }
        }catch(Exception ex){
            out.print("<!--ERROR-->No se pudo actualizar el registro. Informe del error al administrador del sistema: " + ex.toString());
            ex.printStackTrace();
    ...
    if(empresaPermisoAplicacionDto.getRfcPorNipCodigo() == 1 || empresaPermisoAplicacionDto.getRfcPorNipCodigo() == 2){
        ...
        if(!gc.isCelularMexico(celular))
            msgError += "<ul>El dato 'Celular' es requerido. Debe contar con 10 dígitos sin guiones ni espacios.";
        ...
        if (StringManage.getValidString(celular).length()>0){
            if(!gc.isCelularMexico(celular))
                msgError += "<ul>El dato 'celular' es inválido. Deben ser 10 numeros.";
        }
