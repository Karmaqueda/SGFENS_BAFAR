<%-- 
    ...
    Created on : 4/04/2014, 10:45:29 AM
    Author     : leonardo
--%>
...
<%
    ...
    *        22 = Cancelar CFDI
    * 
    *        23 = Agregar Producto pero com impuesto incluido
    *        24 = Agregar Impuesto, este impuesto viene implicito en el concepto
    */
    int mode=-1;
    try{
    ...
    boolean recargarListaImpuestosEnConceptos = false;
    ...
    if (mode==-1){
        ...
        double precio =-1;
        double monto=-1;
        int idTipoComprobante = 41; // 2 = factura
        int idAlmacen = 0;
        ...
        if (msgError.equals("")){
            ...
            try{
                ...
                nuevoProductoCompraSesion.setIdImpuesto(productoCompra.getImpuestoXConcepto());
        ...
        ////////**
        //Hacemos el vaciado de impuestos y vemos cuales estan involucrados aun; ya que quitamos un concepto que puede que tenga impuestos involucrados:
        cfdiSesion.getListaImpuesto().clear();
        //vaciamos totales de impuestos:
        cfdiSesion.setTotalImpuestoTrasladadoEnConcepto(new BigDecimal(0));
        cfdiSesion.setTotalImpuestoRetenidoEnConcepto(new BigDecimal(0));
        
        
        
        for(ProductoSesion productoCompraSesion :cfdiSesion.getListaProducto()){     
            ...
            try{
               productoCompra = conceptoDaoImpl.findByPrimaryKey(productoCompraSesion.getIdProducto());
            }catch(Exception ex){
                msgError = "Ocurrio un error al consultar el producto elegido. " + ex.toString();
            }
            ...
            try{
                ...
                nuevoProductoCompraSesion.setCantidad(productoCompraSesion.getCantidad());
                nuevoProductoCompraSesion.setIdProducto(productoCompraSesion.getIdProducto());
                nuevoProductoCompraSesion.setMonto(productoCompraSesion.getMonto());
                nuevoProductoCompraSesion.setPrecio(productoCompraSesion.getPrecio());
                nuevoProductoCompraSesion.setUnidad(productoCompraSesion.getUnidad());
                nuevoProductoCompraSesion.setIdImpuesto(productoCompra.getImpuestoXConcepto());
                nuevoProductoCompraSesion.setIdAlmacen(productoCompraSesion.getIdAlmacen());
                
              
                
                if (productoCompra!=null){
                    boolean coincidenciaPrevia=false;
                    int indexOcurrencia = 0;
                    for (ProductoSesion itemProducto:cfdiSesion.getListaProducto()){
                        indexOcurrencia++;  
                        if (itemProducto.getIdProducto()==productoCompra.getIdConcepto()){
                            //Ya se había seleccionado este producto
                    ...
                    if (!coincidenciaPrevia){
                        ...
                        ProductoSesion prdExixst= cfdiSesion.getListaProducto().get(indexOcurrencia-1);
                        System.out.println("Existe: " + prdExixst.getIdProducto());
                        System.out.println("Nuevo " + nuevoProductoCompraSesion.getIdProducto());
                        
                        cfdiSesion.getListaProducto().set(indexOcurrencia-1,nuevoProductoCompraSesion);
                        //msgError +="<ul> El producto ya fue seleccionado previamente. Modifique los valores o elimine el producto del listado.";
                    }
                ...
                System.out.println("Prod sess :   ");
                System.out.println("Prod sess :   "  + productoCompraSesion.getIdProducto());
                System.out.println("Prod sess :   ");
                
                ImpuestoPorConceptoBO porConceptoBO = new ImpuestoPorConceptoBO(user.getConn());            
                ImpuestoPorConcepto[] impuestos = porConceptoBO.findImpuestoPorConceptos(0, 0, 0, 0, " AND ID_CONCEPTO = " + productoCompraSesion.getIdProducto() + " AND ID_ESTATUS != 2 ");

                if(impuestos.length>0){
                    for(ImpuestoPorConcepto imp : impuestos){
                ///**
                    //Parametros
                    //int idImpuesto = productoCompra.getIdImpuesto();
                    int idImpuesto = imp.getIdImpuesto();
                    
                        //System.out.println("ID MAYOR A CERO: "+productoCompraSesion.getIdImpuesto());
                        //Parametros
                        //int idImpuesto = productoCompraSesion.getIdImpuesto();
                        ...
                        if (msgError.equals("")){
                            ...
                            try{
                               impuestoDto = impuestoDaoImpl.findByPrimaryKey(idImpuesto);
                                ...
                                msgError = "Ocurrio un error al consultar el impuesto elegido dentro del concepto. " + ex.toString();
                            }
                            ...
                            //cargamos los montos en subtotales de impuestos retenidos y trasladados:                    
                            double totalImpuestoTrasladadoEnConcepto = 0;
                            double totalImpuestoRetenidoEnConcepto = 0;
                            if(nuevoImpuestoCompraSesion.isTrasladado()){
                                totalImpuestoTrasladadoEnConcepto = cfdiSesion.getTotalImpuestoTrasladadoEnConcepto().doubleValue() + (productoCompraSesion.getMonto()*0.01*nuevoImpuestoCompraSesion.getPorcentaje());
                                cfdiSesion.setTotalImpuestoTrasladadoEnConcepto(new BigDecimal(totalImpuestoTrasladadoEnConcepto));
                            }else if(!nuevoImpuestoCompraSesion.isTrasladado()){
                                totalImpuestoRetenidoEnConcepto = cfdiSesion.getTotalImpuestoRetenidoEnConcepto().doubleValue() + (productoCompraSesion.getMonto()*0.01*nuevoImpuestoCompraSesion.getPorcentaje());
                                cfdiSesion.setTotalImpuestoRetenidoEnConcepto(new BigDecimal(totalImpuestoRetenidoEnConcepto));
                            }   
        ...
        System.out.println("Imopuesto add..........."  + cfdiSesion.getListaImpuesto().size() );
        cfdiSesion.setFacturaConConceptosConImpuestos(true);//para decir que es una factura que tiene los impuesto implicitos en el concepto.
        recargarListaImpuestosEnConceptos=true; //para que muestre la tabla de impuestos.
        ////////**    
        ...
        int idTipoComprobante = 1; // 1 = factura
        ...
        ////**GENERAMOS EL XML EN PRODUCCION O EN PRUEBAS:
        String generarEn = request.getParameter("generarEn")!=null?new String(request.getParameter("generarEn").getBytes("ISO-8859-1"),"UTF-8"):"";
        System.out.println("++++++++++++++++++++++++++++++EL COMPROBANTE SE MANDO A GENERAR EN: "+generarEn);
        ...
        if (msgError.equals("")){
            if (cfdiSesion==null){
                ...
                if (cfdiSesion.getListaProducto().size()<=0)
                    msgError+="<ul> Debe seleccionar al menos un producto para generar el Comprobante Fiscal. Lista de Productos y Servicios vacia. ";
                ...
                if (msgError.equals("")){
                    ...
                    //tipo Moneda
                    System.out.println("------------------TIPO DE MONDA: "+tipoMonedaInt);
                    try{
                ...
                if (msgError.equals("")){
                    //---------GENERACION DE XML
                        ...
                        try{
                            ...
                            //ArrayList<mx.bigdata.sat.cfdi.schema.v32.Comprobante.Impuestos.Traslados.Traslado> listaTraslados = new ArrayList<mx.bigdata.sat.cfdi.schema.v32.Comprobante.Impuestos.Traslados.Traslado>();
                            //ArrayList<mx.bigdata.sat.cfdi.schema.v32.Comprobante.Impuestos.Retenciones.Retencion> listaRetenciones = new ArrayList<mx.bigdata.sat.cfdi.schema.v32.Comprobante.Impuestos.Retenciones.Retencion>();
                            ...
                            if(cfdiSesion.isFacturaConConceptosConImpuestos()){
                                //LLAMAMOS LA FUNCION PARA QUE GENERE LOS IMPUESTO NECESARIOS DEL COMPROBANTE:
                                cfdiSesion.getSchemaImpuestosRetenidosTrasladadosFederalesLocalesXConcepto();
                                ...
                                System.out.println("************************* IMPUESTO IMPLICITOS");
                                
                            }else{
                                cfdiSesion.getSchemaImpuestosRetenidosTrasladadosFederales();
                            ...
                            double totalComprobante = cfdiSesion.calculaTotal().doubleValue();
                            
                            ///*para enviar al total el total de los impuestos implisitos en el concepto.
                            if(cfdiSesion.isFacturaConConceptosConImpuestos()){
                                totalComprobante = cfdiSesion.calculaTotalImpuestoEnConceptos().doubleValue();
                            }
                            ///*
                            ...
                            //***COMO NO SE GENERO CORRECTAMENTE EL XML, REGRERSAMOS EL CONTADOR DE FOLIO:
                                    /*if (idFolios>0){
                                        int folioGenerado2 = -1;
                                        try{
                                            Folios foliosDto = new FoliosBO(idFolios, user.getConn()).getFolios();
                                            if (foliosDto!=null){
                                                folioGenerado2 = foliosDto.getUltimoFolio() - 1;
                                                    foliosDto.setUltimoFolio(folioGenerado2);
                                                    try{
                                                        new FoliosDaoImpl(user.getConn()).update(foliosDto.createPk(), foliosDto);
                                                    }catch(Exception ex2){}                                                
                                            }else{
                ...
                if (msgError.trim().equals("")){
                    if (cfdiSesion.getIdPedido()<=0){
                        try{
                            ...
                            System.out.println("No se pudo descontar existencia de almacen");
                        }
                    }
                }
                ...
                if (msgError.equals("") && archivoXML!=null && cfd32BO!=null){
                    ...
                    if (msgError.equals("")){
                        ...
                        try{
                            cfdiSesion.setGuardarEnImpuestosXconcepto(1);//la pasamos el parametro de 1 para que almacene los impuestos en la tabla de COMPROBANTE_IMPUESTO_X_CONCEPTO
                            ...
                            /*try{
                                ...
                                folioGenerado = foliosDto.getUltimoFolio() - 1;
                                foliosDto.setUltimoFolio(folioGenerado);
                                new FoliosDaoImpl().update(foliosDto.createPk(), foliosDto);
                            }catch(Exception ex2){}*/
                    ...
                    if (cfdiSesion.getIdPedido()>0){
                        try{
                                ...
                                if (pedidoDto!=null){
                                    ...
                                    new SgfensPedidoDaoImpl().update(pedidoDto.createPk(), pedidoDto);
                                }else{
        ...
        //Modo 23 = agregar producto e impuestos relacionados al producto
        ...
        try{ precio = Double.parseDouble(request.getParameter("producto_precio")); }catch(Exception e){}
        try{ monto = Double.parseDouble(request.getParameter("producto_monto")); }catch(Exception e){}
        try{ productoDescuento = Double.parseDouble(request.getParameter("productoDescuento")); }catch(Exception e){}
        try{ idTipoComprobante = Integer.parseInt(request.getParameter("id_tipo_comprobante")); }catch(Exception e){}
        try{ idAlmacen = Integer.parseInt(request.getParameter("almacen_id")); }catch(Exception e){}
        almacenNombre = request.getParameter("almacenNombre") != null ? request.getParameter("almacenNombre") : "";
        ...
        if (msgError.equals("")){
            ...
            try{
                ...
                nuevoProductoCompraSesion.setIdImpuesto(productoCompra.getImpuestoXConcepto());
                nuevoProductoCompraSesion.setIdAlmacen(idAlmacen);
                if(productoDescuento > 0)
                    nuevoProductoCompraSesion.setDescuento_tasa(productoDescuento);
        ...
        //Modo 24 = Agregar Impuesto
        if(productoCompra.getImpuestoXConcepto()>0){//SI ES MAYOR A 0 ES QUE TIENE IMPUESTO RELACIONADOS:
            ///**RECUPERAMOS LOS IMPUESTOS RELACIONADOS A DICHO CONCEPTO
            ImpuestoPorConceptoBO porConceptoBO = new ImpuestoPorConceptoBO(user.getConn());            
            ImpuestoPorConcepto[] impuestos = porConceptoBO.findImpuestoPorConceptos(0, 0, 0, 0, " AND ID_CONCEPTO = " + idProducto + " AND ID_ESTATUS != 2 ");
            
            System.out.println("------------IMPUESTO IMPLICITOS: "+impuestos.length);
            ...
            ///**
                    ...
                    //Parametros
                    //int idImpuesto = productoCompra.getImpuestoXConcepto();
                    int idImpuesto = imp.getIdImpuesto();
                    ...
                    if (msgError.equals("")){
                        ...
                        double totalImpuestoRetenidoEnConcepto = 0;
                        if(nuevoImpuestoCompraSesion.isTrasladado()){
                            totalImpuestoTrasladadoEnConcepto = cfdiSesion.getTotalImpuestoTrasladadoEnConcepto().doubleValue() + (nuevoProductoCompraSesion.getMonto()*0.01*nuevoImpuestoCompraSesion.getPorcentaje());
                            cfdiSesion.setTotalImpuestoTrasladadoEnConcepto(new BigDecimal(totalImpuestoTrasladadoEnConcepto));
                        }else if(!nuevoImpuestoCompraSesion.isTrasladado()){
                            totalImpuestoRetenidoEnConcepto = cfdiSesion.getTotalImpuestoRetenidoEnConcepto().doubleValue() + (nuevoProductoCompraSesion.getMonto()*0.01*nuevoImpuestoCompraSesion.getPorcentaje());
                            cfdiSesion.setTotalImpuestoRetenidoEnConcepto(new BigDecimal(totalImpuestoRetenidoEnConcepto));
                        }   
        ...
        for(ProductoSesion productoCompraSesion :cfdiSesion.getListaProducto()){
            if(productoCompraSesion.getIdImpuesto()>0){//SI ES MAYOR A 0 ES QUE TIENE IMPUESTO RELACIONADOS:
                ///**RECUPERAMOS LOS IMPUESTOS RELACIONADOS A DICHO CONCEPTO
                ImpuestoPorConceptoBO porConceptoBO = new ImpuestoPorConceptoBO(user.getConn());            
                ImpuestoPorConcepto[] impuestos = porConceptoBO.findImpuestoPorConceptos(0, 0, 0, 0, " AND ID_CONCEPTO = " + productoCompraSesion.getIdProducto() + " AND ID_ESTATUS != 2 ");
...
%>
                        <tr>
                            <td>
                                ...
                                <input type="text" maxlength="100" name="producto_nombre_<%=id%>" id="producto_nombre_<%=id%>" size="16" readonly
                                       disabled value="<%=conceptoBO.getNombreConceptoLegible() %>"/>
                                
                            </td>
                            <td>
                                <textarea name="producto_descripcion_<%=id%>" id="producto_descripcion_<%=id%>" 
                                       rows="3" cols="30" readonly
                            ...
                            <td>
                                <input type="text" maxlength="100" name="producto_unidad_<%=id%>" id="producto_unidad_<%=id%>" size="16" readonly
                            ...
                            <td>
                                <input type="text" maxlength="10" name="producto_precio_<%=id%>" id="producto_precio_<%=id%>" size="8"  style="text-align: right;" readonly
                                       disabled value="<%=itemProductoSesion.getPrecio() %>"/>
                            </td>
                            <td>
                                <input type="text" maxlength="8" name="producto_cantidad_<%=id%>" id="producto_cantidad_<%=id%>" style="text-align: right;" size="5"
                                       disabled value="<%=itemProductoSesion.getCantidad() %>"/>
                            </td>
                            <td>
                                <input type="text" maxlength="16" name="producto_descuento_<%=id%>" id="producto_descuento_<%=id%>" style="text-align: right;" size="5"
                                       disabled value="<%=itemProductoSesion.getDescuento_tasa() %>"/>
                            </td>
                            <td>
...
<%
    ...
    if(recargarListaImpuestosEnConceptos){
        System.out.println("----------RECARGANDO IMPUESTOS");
        if (msgError.equals("")){
%>
                <table class="data" width="100%" cellpadding="0" cellspacing="0">
                    <!--<thead>
...
<%     /*         }*/%>
                        <tr style="text-align: right;">
                            <td colspan="3">
                                <h3>Subtotal Traslados (+):
                                <input type="text" maxlength="16" name="subtotalImpuestos_trasladosEnConcepto" id="subtotalImpuestos_trasladosEnConcepto" style="text-align: right;" size="10"
                                    readonly value="<%= cfdiSesion.getTotalImpuestoTrasladadoEnConcepto() %>"/>
                                </h3>
                            </td>
                        </tr>
                        <tr style="text-align: right;">
                            <td colspan="3">
                                <h3>Subtotal Retenciones (-):
                                <input type="text" maxlength="16" name="subtotalImpuestos_retencionesEnConcepto" id="subtotalImpuestos_retencionesEnConcepto" style="text-align: right;" size="10"
                                    readonly value="<%= cfdiSesion.getTotalImpuestoRetenidoEnConcepto() %>"/>
                                </h3>
                            </td>
                        </tr>
                </tbody>
