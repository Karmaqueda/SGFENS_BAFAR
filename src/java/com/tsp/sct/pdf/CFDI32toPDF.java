...
public class CFDI32toPDF {
     ...
     * @throws Exception
     */
    public ByteArrayOutputStream toPdf_() throws Exception{
        ...
        try{
            ...
            Comprobante.Complemento comp = comprobanteFiscal.getComplemento();
            if (comp != null) {
            ...
            Cfdi32ToPdfEventExt event = new Cfdi32ToPdfEventExt(document, comprobanteFiscal, LETRAOCHO, LETRAOCHO_BOLD,timbreFiscalDigital, certificadoPAC, comprobanteFiscalDto);
            ...
            PdfPTable tAux = new PdfPTable(1);
                    ...
                    tConceptos.setLockedWidth(true);
                        ...
                        Iterator<mx.bigdata.sat.cfdi.schema.v32.Comprobante.Conceptos.Concepto> cfdiConceptoItera = comprobanteFiscal.getConceptos().getConcepto().iterator();
                    ...
                    tTotal.setLockedWidth(true);
                        ...
                        try{
                            if (comprobanteFiscal.getMoneda()!=null){
                                if (!"".equals(comprobanteFiscal.getMoneda().trim())){
                                    numALetra.setNombreMoneda(comprobanteFiscal.getMoneda().trim());
                                }
                            }
                            //totalConLetra = numALetra.getStringOfNumber(comprobanteCFDI.getTotal().floatValue());
                            totalConLetra = numALetra.getStringOfNumber(comprobanteFiscal.getTotal());
                        }catch(Exception e){}
                        ...
                        tAux.setTotalWidth(150);
                            ...
                            if(comprobanteFiscal.getImpuestos().getTraslados()!=null){
                                Iterator<mx.bigdata.sat.cfdi.schema.v32.Comprobante.Impuestos.Traslados.Traslado> iteraTraCFDI = comprobanteFiscal.getImpuestos().getTraslados().getTraslado().iterator();
                            ...
                            if(comprobanteFiscal.getImpuestos().getRetenciones()!=null){
                                Iterator<mx.bigdata.sat.cfdi.schema.v32.Comprobante.Impuestos.Retenciones.Retencion> iteraRetCFDI = comprobanteFiscal.getImpuestos().getRetenciones().getRetencion().iterator();
                                while(iteraRetCFDI.hasNext()){
                                    ...
                                    double tasa=0;
                                    tasa = (retencionCDFI.getImporte().doubleValue()/comprobanteFiscal.getSubTotal().doubleValue())*100;
                                    obj.agregaCelda(tAux,LETRASIETE_BOLD, "RETENCIÓN " + retencionCDFI.getImpuesto() + " " + tasa + "%" , new boolean[]{false,false,false,false}, Element.ALIGN_LEFT, Element.ALIGN_TOP, 0, new int[0],1);
