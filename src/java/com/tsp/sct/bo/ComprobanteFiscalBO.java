...
import com.tsp.sct.dao.jdbc.AduanaTercerosDaoImpl;
...
public class ComprobanteFiscalBO {
    ...
    public ComprobanteFiscalBO(Connection conn){
        this.conn = conn;
    }
    
    public ComprobanteFiscalBO(long idComprobanteFiscal, Connection conn){
        this.conn = conn;
        try{
    ...
    public ComprobanteFiscalBO(int idComprobanteFiscal, Connection conn){
        this.conn = conn;
        try{
            ComprobanteFiscalDaoImpl ComprobanteFiscalDaoImpl = new ComprobanteFiscalDaoImpl(this.conn);
        ...
        try{
            ...
            for (ComprobanteDescripcion itemProducto:listaProductosCompra){
                try{
                    ...
                    try{
                        aduanas = null;
                        aduanas = aduanaDaoImpl.findWhereIdComprobanteDescripcionEquals(itemProducto.getIdComprobanteDescripcion());
                        if(aduanas != null){
                            for(Aduana adu : aduanas){
                                productoSesion.getAduanas().add(adu);
                            }
                        }
                    }catch(Exception e){}
    ...
    public ComprobanteFiscal creaActualizaComprobanteFiscal(ComprobanteFiscalSesion comprobanteFiscalSesion, UsuarioBO user) throws Exception{
        ...
        //Crear
            ...
            //Crear Notificacion SMS - solo en caso de Nuevo Comprobante Fiscal
            try{
                EmpresaPermisoAplicacion empresaPermisoAplicacion 
                    = new EmpresaPermisoAplicacionDaoImpl(conn).findByPrimaryKey(new EmpresaBO(conn).getEmpresaMatriz(comprobanteFiscalDto.getIdEmpresa()).getIdEmpresa()); 
                ...
                // Solo si la empresa matriz tiene contratado el modulo SMS
                    ...
                    smsEnvioLoteBO.crearSmsNotificacionSistema(SmsEnvioLoteBO.TipoNotificaSistema.FACTURA_NUEVO, comprobanteFiscalDto.getIdEmpresa(), comprobanteFiscalDto.getIdComprobanteFiscal());
                }
            }catch(Exception ex){
                ex.printStackTrace();
            }
        ...
        for (ProductoSesion prodSesion:comprobanteFiscalSesion.getListaProducto()){
            ...
            try{
                ...
                ComprobanteDescripcionPk comprobanteDescripcionPK = comprobanteDescripcionDao.insert(prodComprobanteFiscal);
                
                for(Aduana adu : prodSesion.getAduanas()){
                    
                    int nextIdAduana = -1;
                    try{
                        Aduana lastAduana = aduanaDaoImpl.findLast();
                        if (lastAduana!=null){
                            nextIdAduana = aduanaDaoImpl.findLast().getIdAduana() + 1;
                        }else{
                            //Ocurre solamente cuando la base de datos no tiene ningun registro de Comprobante Fiscal
                            nextIdAduana = 1;
                        }
                    ...
                    try{
                        ...
                        aduana.setIdAduana(nextIdAduana);
                        aduana.setIdComprobanteDescripcion(comprobanteDescripcionPK.getIdComprobanteDescripcion());
                        aduana.setIdComprobanteFiscal(comprobanteFiscalDto.getIdComprobanteFiscal());
                        aduana.setNumDocumento(adu.getNumDocumento());
                        aduana.setFechaExpedicion(adu.getFechaExpedicion());
                        aduana.setAduana(adu.getAduana());
                        aduanaDaoImpl.insert(aduana);
                    }catch(Exception e){
                        e.printStackTrace();
                    }
                }
...
//Editado 02/10/2013, conexion a CMM
                if (!respCancela.isIsError() && envioNetAcuse!=null){
...
*/
        ...
        if (exito){
            ...
            //Crear Notificacion SMS - solo en caso de cancelacion exitosa
            try{
                EmpresaPermisoAplicacion empresaPermisoAplicacion 
                    = new EmpresaPermisoAplicacionDaoImpl(conn).findByPrimaryKey(new EmpresaBO(conn).getEmpresaMatriz(comprobanteFiscal.getIdEmpresa()).getIdEmpresa()); 
                if (empresaPermisoAplicacion!=null && empresaPermisoAplicacion.getAccesoModuloSms()==1){
                // Solo si la empresa matriz tiene contratado el modulo SMS
                    SmsEnvioLoteBO smsEnvioLoteBO = new SmsEnvioLoteBO(conn);
                    smsEnvioLoteBO.crearSmsNotificacionSistema(SmsEnvioLoteBO.TipoNotificaSistema.FACTURA_CANCELAR, comprobanteFiscal.getIdEmpresa(), comprobanteFiscal.getIdComprobanteFiscal());
                }
            }catch(Exception ex){
                ex.printStackTrace();
            }
