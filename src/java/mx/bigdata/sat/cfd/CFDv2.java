...
public final class CFDv2 implements CFD {
  ...
  public String sellarPSGECFD(PrivateKey key, X509Certificate cert) throws Exception {
    cert.checkValidity();
    String signature = getSignature(key);
    List<Object> listComplementos = document.getComplemento().getAny();
    if (listComplementos!=null && listComplementos.size()>0) {
        try {
            String auxComplemento = (String)listComplementos.get(0);
        }
        catch (Exception e) {
            String auxError = e.toString();
        }
    }
    return signature;
  }
  ...
  public String getNoCertificado() {
    ...
    return document.getNoCertificado();
  }
}